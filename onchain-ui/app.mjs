...
function setGo() {
  ...
  paintAlt();
  go.disabled = !deployed || !implOk || lock || !ready || !destOk;
  ...
  go.title =
    busy || !go.disabled
      ? ''
      : pending
        ? 'A send is still confirming. Check Activity.'
        : !eoa
          ? 'Connect wallet to send'
          : !deployed
            ? 'Create your account first'
            : !implOk
              ? 'Unexpected implementation. Don’t send.'
              : noGas
                ? EMPTY_WALLET
                : !destOk
                  ? 'Enter a destination.'
                  : 'Pick assets and enter amounts.';
}
...
function setStatus(s, cls) {
  ...
  if (el.textContent === next && el.className === clsName) return;
  el.textContent = next;
...
async function pickRpc(id) {
  ...
  const tryUrl = async (u) => {
    ...
    // chainId-only lets a free-tier node win then fail on balance/logs
...
async function ensureRpc() {
  ...
  if (custom && custom !== getRpcUrl()) {
    setRpcUrl(await pickRpc(chainId));
    return;
  }
  if (getRpcUrl()) {
    try {
...
async function scanCtx() {
  ...
  if (fork) {
    ...
    try {
      ...
      // non-archive public nodes fail the code walk; 120k blocks still finds recent holdings
      birth = Math.max(0, latest - 120000);
    }
  }
...
async function refreshAccount(opts) {
  ...
  implOk = true;
  if (deployed) {
    const impl = '0x' + String(await jrpc(getRpcUrl(), 'eth_getStorageAt', [aa, EIP1967_IMPL_SLOT, 'latest'])).slice(-40);
    implOk = eq(impl, KERNEL_IMPL);
  }
  showSkill();
  const note = !implOk && deployed
    ? 'Unexpected implementation. Don’t send.'
    : deployed && legacyStranded
      ? CREATE_STRANDED
      : deployed && altAa && aaSource === 'ours'
        ? ALSO_OLDER
        : deployed && altAa && aaSource === 'legacy'
          ? ALSO_OURS
          : deployed
            ? ''
            : !aa
              ...
              : aaRescue
                ? CREATE_RESCUE
                : CREATE_NOTE;
  $('aaNote').textContent = note;
  $('aaNote').className = !implOk || (!deployed && (eoaWei === 0n || !aa)) ? 'err' : 'muted';
  paintAlt();
  ...
  const scanP = ctxP.then(async (c) => {
    ...
    if (from > c.latest) return { ...c, logs: { ops: [], ins: [], outs: [], wds: [], rcv: [] } };
