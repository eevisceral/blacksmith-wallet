...
}
{
  ...
  try {
    ...
    // Timed-out URL fails over immediately (scanLogs owns rotation).
    if (got.length !== 1 || hits.join(' ') !== 'https://one.invalid https://two.invalid') {
      console.error('FAIL getLogs must treat a timeout as retryable and fail over', hits);
      process.exit(1);
    }
    // 429 HTML: do not retry the same URL; fail over and promote for later reads.
    stub((u) => {
      if (u === 'https://one.invalid') return { ok: false, status: 429, text: async () => '<html>Too Many Requests</html>' };
      return res([log]);
    });
    setRpcUrl('https://one.invalid', ['https://two.invalid']);
    const r429 = await getLogs(10, 20, [TRANSFER_TOPIC]);
    if (r429.length !== 1 || hits.filter((u) => u === 'https://one.invalid').length !== 1 || hits[hits.length - 1] !== 'https://two.invalid') {
      console.error('FAIL 429 must fail over without retrying the same URL', hits);
      process.exit(1);
    }
    stub((u) => {
      if (u === 'https://one.invalid') return { ok: false, status: 429, text: async () => '<html>Too Many Requests</html>' };
      return { ok: true, text: async () => JSON.stringify({ jsonrpc: '2.0', id: 1, result: '0x1' }) };
    });
    setRpcUrl('https://one.invalid', ['https://two.invalid']);
    const cv = await call('0x' + '11'.repeat(20), '0x');
    if (cv !== '0x1' || getRpcUrl() !== 'https://two.invalid' || hits.filter((u) => u === 'https://one.invalid').length !== 1) {
      console.error('FAIL call 429 must promote alt without same-URL retry', hits, getRpcUrl(), cv);
      process.exit(1);
    }
    if (!transientRpcMsg('Failed to fetch')) {
      console.error('FAIL transientRpcMsg must catch Failed to fetch');
      process.exit(1);
    }
    ...
    if (wide.length !== 2 || hits.filter((u) => u === 'https://one.invalid').length !== 1 || hits[hits.length - 1] !== 'https://two.invalid') {
    ...
    // Exclusive custom URL, no public fall-through.
    if (msg !== LOGS_TIMEOUT_CUSTOM || hits.length !== 1 || hits[0] !== 'https://custom.invalid') {
...

{
  ...
  try {
    ...
    // Hung RPCs abort when the clock is spent — they must not wait out 30s × every URL.
    setRpcUrl('https://hang.invalid', ['https://hang2.invalid', 'https://hang3.invalid']);
    globalThis.fetch = (u, init) =>
      new Promise((_, rej) => {
        const s = init && init.signal;
        const t = setTimeout(() => rej(new DOMException('signal timed out', 'TimeoutError')), 60000);
        const boom = () => {
          clearTimeout(t);
          rej((s && s.reason) || new DOMException('The operation was aborted', 'AbortError'));
        };
        if (s) {
          if (s.aborted) boom();
          else s.addEventListener('abort', boom);
        }
      });
    const tHang = Date.now();
    const wHang = await walkTransfersIn(1, 50000, AA2, {
      budget: { pages: 30, ms: 2500, t0: Date.now(), used: 0 },
      descend: true,
    });
    const dtHang = Date.now() - tHang;
    if (wHang.done || wHang.stopped !== 'ms' || dtHang > 6000) {
      console.error('FAIL budgeted walk must stop a hung RPC inside the time budget', wHang.stopped, dtHang);
      process.exit(1);
    }
    setRpcUrl('https://deep.invalid', []);
