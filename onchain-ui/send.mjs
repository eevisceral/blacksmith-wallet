...
export async function send() {
  ...
  try {
    ...
    if (!S.implOk()) throw new Error('Unexpected implementation. Don’t send.');
    if (calls.length > S.maxPicks()) throw new Error(S.sendOne);
    const callData =
...
export async function createKernel() {
  ...
  try {
    ...
    if (hasCode(code)) {
      if (!kernelExecImpl(await kernelImpl(aa))) throw new Error('Account created with an unexpected implementation. Don’t send.');
